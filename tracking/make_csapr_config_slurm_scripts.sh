...
# Slurm template
slurm_template=${slurm_dir}slurm_csapr${resolution}_template.sh
# Config template
config_template=${config_dir}config_csapr${resolution}_template.yml
...
for ((i = 0; i < ${#start_dates[@]}; ++i)); do   
    ...
    slurm_file=${slurm_dir}${slurm_basename}${sdate}.sh

    sed "s|STARTDATE|$sdate|g; s|ENDDATE|$edate|g" ${config_template} > ${config_file}
    sed "s|STARTDATE|$sdate|g; s|CONFIG_NAME|$config_name|g; s|CONFIG_FILE|$config_file|g; s|FLEXTRKR_DIR|$pyflex_dir|g" ${slurm_template} > ${slurm_file}
    echo ${config_file}
    echo ${slurm_file}
