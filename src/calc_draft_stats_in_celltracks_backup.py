...
if __name__ == '__main__':
    ...
    # Enhanced progress monitoring and batch processing
    print(f"Starting processing of {nfiles} files...")
    start_time = time.time()
    last_progress_time = start_time
    max_concurrent_tasks = min(10, n_workers * 2) if run_parallel == 1 else 1
        ...
        if ifile % 20 == 0:
            ...
            if run_parallel == 1:
                try:
                    ...
                    pass  # Continue if cluster info not available
    ...
    if run_parallel == 1 and len(final_results) > 0:
        ...
        try:
            ...
            if delayed_results:
                ...
                for chunk_start in range(0, len(delayed_results), chunk_size):
                    ...
                    print(f"Computing chunk {chunk_start//chunk_size + 1}/{(len(delayed_results)-1)//chunk_size + 1} "
                    ...
                    try:
                        ...
                        print(f"Chunk failed: {e}, processing items individually...")
                        for item in chunk:
                            try:
                                computed_results.append(item.compute())
                            except Exception as e2:
                                print(f"Individual item failed: {e2}")
        ...
        try:
            ...
            print(f"Error closing Dask cluster: {e}")
