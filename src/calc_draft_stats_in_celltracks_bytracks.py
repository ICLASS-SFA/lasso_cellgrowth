...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print("Usage: python calc_draft_stats_in_celltracks_bytracks.py config_file.yml track_start track_end [digits]")
        print("Example: python calc_draft_stats_in_celltracks_bytracks.py config.yml 0 99")
        print("Example: python calc_draft_stats_in_celltracks_bytracks.py config.yml 0 99 4")
        print("  digits: Number of digits for track start padding (default: 4)")
        sys.exit(1)

    # Get configuration file name and track range from input
    config_file = sys.argv[1]
    track_start = int(sys.argv[2])
    track_end = int(sys.argv[3])
    
    # Get digits for padding (default 4)
    if len(sys.argv) == 5:
        digits = int(sys.argv[4])
    else:
        digits = 4  # Default to 4 digits
    
    print(f"Processing tracks {track_start} to {track_end} with {digits}-digit padding")
    ...
    output_path = config['output_path'] + 'tmp/' # Add tmp/ for intermediate output files
    ...
    # Output statistics filename with track range suffix
    output_basename = 'stats_3d_w_fixshell_'
    track_start_str = str(track_start).zfill(digits)
    output_filename = f'{output_path}{output_basename}{startdate}_{enddate}_t{track_start_str}.nc'
    ...
    # Get total number of tracks before subsetting
    ntracks_total = dsstats.sizes[tracks_dimname]
    print(f'Total Number of Tracks in file: {ntracks_total}')
    
    # Validate track range
    if track_start >= ntracks_total:
        print(f"ERROR: track_start ({track_start}) is >= total tracks ({ntracks_total})")
        sys.exit(1)
    
    # Adjust track_end if it exceeds total tracks
    track_end_actual = min(track_end, ntracks_total - 1)
    if track_end_actual != track_end:
        print(f"Adjusted track_end from {track_end} to {track_end_actual} (total tracks: {ntracks_total})")
    
    # Subset the track statistics to the specified range
    dsstats = dsstats.sel(tracks=slice(track_start, track_end_actual + 1))
    ...
    print(f'Processing {ntracks} tracks (indices {track_start} to {track_end_actual})')
    ...
    # Note: stats_basetime is already subsetted when we read the track stats file
    ...
    gattr_dict = {
        'title':  f'Tracked cell W statistics (tracks {track_start}-{track_end_actual})', \
        ...
        'startdate': startdate, \
        'enddate': enddate, \
        'track_range': f'{track_start}-{track_end_actual}', \
    ...
    print(f"Processing completed successfully for tracks {track_start} to {track_end_actual}")