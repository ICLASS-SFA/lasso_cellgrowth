...
def extract_env_prof(
    ...
    if (nmatchcell > 0):
        ...
        for itrack in range(0, ntracks):
            ...
            # Cloud file
            if cld_exist:
                lat_idx, lon_idx = location_to_idx(dsc['XLAT'], dsc['XLONG'], center)
                _LWP = pad_array(dsc['LWP'].squeeze().data, lat_idx, lon_idx, ny, nx, ny_c, nx_c, sub_y, sub_x)
                _IWP = pad_array(dsc['IWP'].squeeze().data, lat_idx, lon_idx, ny, nx, ny_c, nx_c, sub_y, sub_x)
                _pwv = pad_array(pwv.squeeze().data, lat_idx, lon_idx, ny, nx, ny_d, nx_d, sub_y, sub_x)
            ...
            if wrfout_exist:
                ...
                lat_idx, lon_idx = location_to_idx(XLAT, XLONG, center)
...
if __name__ == '__main__':
    ...
    # while counter < len(final_results):
    while counter > 0:
